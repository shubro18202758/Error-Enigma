...
class ContentProcessor:
    ...
    def process_video_content(self, course_details: CourseDetails, 
                            module_details: ModuleDetails,
                            video_file_path: str) -> ProcessedContent:
        """
        Process video content into structured microlearning format
        ...
        try:
            self.logger.info(f"Starting content processing for: {video_file_path}")
            
            # Step 1: Extract video metadata
            self._update_status(content_id, 0.1, "Extracting video metadata...")
            video_metadata = self._extract_video_metadata(video_file_path)
            
            # Step 2: Transcribe video
            self._update_status(content_id, 0.2, "Transcribing video content...")
            transcription = self.transcription_service.transcribe_video(video_file_path)
