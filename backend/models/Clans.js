...
// Simplified Clans model with proper database calls
...
class Clans {
  ...
  static async respondToInvitation(invitationId, userId, response) {
    try {
      const getInvitationStmt = db.prepare(`
        SELECT * FROM clan_invitations WHERE id = ? AND invited_user_id = ?
      `);
      const invitation = getInvitationStmt.get(invitationId, userId);
      ...
      `);
      updateStmt.run(response === 'accept' ? 'accepted' : 'declined', invitationId);
  ...
  static async addActivity(clanId, userId, activityType, activityData = {}, pointsEarned = 0) {
    try {
      const stmt = db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?)
      `);
      stmt.run(clanId, userId, activityType, JSON.stringify(activityData), pointsEarned);

      // Update clan total points
      const updateClanStmt = db.prepare(`
        UPDATE clans SET total_points = total_points + ? WHERE id = ?
      `);
      updateClanStmt.run(pointsEarned, clanId);
      ...
      `);
      updateMemberStmt.run(pointsEarned, clanId, userId);
  ...
  static async leaveClan(clanId, userId) {
    try {
      ...
      `);
      deleteStmt.run(clanId, userId);

      // Update member count
      const updateStmt = db.prepare(`
        UPDATE clans SET member_count = member_count - 1 WHERE id = ?
      `);
      updateStmt.run(clanId);
